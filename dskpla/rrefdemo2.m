...
%
% rrefdemo2函数用高斯消元法将方阵化简为对角矩阵，
% 过程中既进行行交换，也进行列交换找最大主元 
% 用高斯消元法找到方程组的解 [U,L]= rrefdemo2(A) 
% 线性方程组消元全过程显示
...
for k = 1:m-1
    ...
    if (isav ~= k)
        % 行交换
        fprintf('先进行行交换，把系数矩阵右下方中绝对值最大元素A(%d,%d)移到主元行同列A(%d,%d)的位置\n',isav,jsav,k,jsav)
        for j = 1:n
            temp = A(k,j);
            temp1 = A(isav,j);
            A(k,j) = temp1;
    ...
    if (jsav ~= k)
        fprintf('再进行列交换，使该绝对值最大元素A(%d,%d)移到主元A(%d,%d)的位置\n',k,jsav,k,k)
        % 列交换
        tempcol = col(jsav);
        col(jsav) = col(k);
        col(k) = tempcol;
...
if n==m
    disp('方程常数项不存在，增广矩阵不全，不可解'),return
else
    for j = 1:n-m
        for i = 1:m
            xo(j,col(i)) = x(j,i);
