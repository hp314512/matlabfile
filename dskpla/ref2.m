% 《实用大众线性代数（MATLAB版）》第一章化为对角行阶梯形式子程序ref2
% 
function [A,jb] = ref2(A,tol)
%REF2   求矩阵的对角元素未归一化的最简行阶梯形式
%   R = REF2(A)  生成A的对角未归一化的行阶梯形式.
%
%   [R,jb] = REF2(A) 同时返回数组 jb:
%       r = length(jb) 是A的秩。
%       x(jb) 为线性系统Ax = b中约束变量, ,
%       A(:,jb) 是A空间中的一组基,
%       R(1:r,jb) 是上三角矩阵.
%
%   [R,jb] = REF2(A,TOL) 按给定的容差判断秩.
%
